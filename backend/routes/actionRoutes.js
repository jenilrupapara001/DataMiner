...
router.post('/:id/start', protect, requireAnyPermission(['actions_edit', 'actions_manage']), async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`UPDATE Actions SET Stage = @Stage, TimeTracking = @TimeTracking, UpdatedAt = DATEADD(minute, 330, GETUTCDATE()) WHERE Id = @id`);
...
router.post('/:id/submit-review', protect, requireAnyPermission(['actions_edit', 'actions_manage']), upload.single('audio'), async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`UPDATE Actions SET Completion = @Completion, Stage = @Stage, Status = @Status, UpdatedAt = DATEADD(minute, 330, GETUTCDATE()) WHERE Id = @id`);
    ...
    try {
        ...
        if (decision === 'APPROVE') {
            ...
            await pool.request()
                ...
                .input('Stage', sql.NVarChar, JSON.stringify(stageHistory))
                .input('Status', sql.NVarChar, 'COMPLETED')
                .query(`UPDATE Actions SET Stage = @Stage, Status = @Status, UpdatedAt = DATEADD(minute, 330, GETUTCDATE()) WHERE Id = @id`);
            ...
            await pool.request()
                .input('id', sql.VarChar, req.params.id)
                .input('Stage', sql.NVarChar, JSON.stringify(stageHistory))
                .query(`UPDATE Actions SET Stage = @Stage, Status = 'PENDING', UpdatedAt = DATEADD(minute, 330, GETUTCDATE()) WHERE Id = @id`);
        }
        ...
        for (const actionData of suggestedActions) {
            ...
            await pool.request()
                ...
                .query(`
                    ...
                    VALUES (@Id, @Title, @Description, @Type, @Priority, @Status, @Asins, @SellerId, @CreatedBy, @AutoGenerated, @TimeTracking, DATEADD(minute, 330, GETUTCDATE()), DATEADD(minute, 330, GETUTCDATE()))
                `);
...
router.post('/:id/upload-audio', protect, requireAnyPermission(['actions_edit', 'actions_manage']), upload.single('audio'), async (req, res) => {
    try {
        ...
        await pool.request()
            .input('id', sql.VarChar, req.params.id)
            .input('Completion', sql.NVarChar, JSON.stringify(completion))
            .query(`UPDATE Actions SET Completion = @Completion, UpdatedAt = DATEADD(minute, 330, GETUTCDATE()) WHERE Id = @id`);
        ...
        const result = await pool.request()
            .query(`
                ...
                WHERE a.Status != 'COMPLETED'
                  ...
                  AND a.DueDate < DATEADD(minute, 330, GETUTCDATE())
            `);

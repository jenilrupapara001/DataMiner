...
exports.getActions = async (req, res) => {
    try {
        const {
            status, priority, assignedTo, stage,
        ...
        if (status) whereClauses.push(`a.Status = '${status}'`);
        if (priority) whereClauses.push(`a.Priority = '${priority}'`);
        if (assignedTo) whereClauses.push(`a.AssignedTo = '${assignedTo}'`);

        // Full-text search on title/description
        if (search && search.trim()) {
            const safe = search.replace(/'/g, "''").trim();
            whereClauses.push(`(a.Title LIKE '%${safe}%' OR a.Description LIKE '%${safe}%')`);
        }

        // Data isolation for non-global users (no LIKE subquery — join-based)
        if (!isGlobalUser) {
            ...
            const assignedSellerIds = (req.user.assignedSellers || []).map(s => (s._id || s).toString());
            const sellerList = assignedSellerIds.length > 0
                ? assignedSellerIds.map(id => `'${id}'`).join(',')
                : "'__NONE__'";
        ...
        const [actionsResult, countResult] = await Promise.all([
            pool.request().query(`
                SELECT
                    a.Id, a.Title, a.Description, a.Status, a.Priority, a.Type,
                    a.AssignedTo, a.CreatedBy, a.SellerId, a.Asins, a.Stage,
                    a.Completion, a.TimeTracking, a.Recurring, a.AutoGenerated,
                    ...
                    s.Marketplace as sellerMarketplace
                ${baseQuery}
                ORDER BY a.CreatedAt DESC
                OFFSET ${offset} ROWS FETCH NEXT ${limitNum} ROWS ONLY
            `),
            pool.request().query(`SELECT COUNT(*) as total ${baseQuery}`)
        ]);
        ...
        res.status(200).json({ success: true, data: [], pagination: { page: 1, limit: 50, total: 0, totalPages: 0, hasMore: false } });
    }
};

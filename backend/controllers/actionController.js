        ...
        for (const [, group] of Object.entries(bySellerType)) {
            ...
            await pool.request()
                ...
                .query(`
                    ...
                    VALUES (@Id, @Type, @Title, @Description, @Priority, 'PENDING', @AsinsJson, @SellerId, @CreatedBy, @AutoGenerated, @TimeTracking, DATEADD(minute, 330, GETUTCDATE()), DATEADD(minute, 330, GETUTCDATE()))
                `);
            ...
            for (let i = 0; i < timeframe; i++) {
                ...
                await pool.request()
                    ...
                    .query(`
                        ...
                        VALUES (@Id, @Title, @Description, @Type, @Priority, @Status, @Asins, @SellerId, @AssignedTo, @CreatedBy, @DueDate, @Stage, @TimeTracking, DATEADD(minute, 330, GETUTCDATE()), DATEADD(minute, 330, GETUTCDATE()))
                    `);
...
exports.createTemplate = async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .query(`INSERT INTO TaskTemplates (Id, Title, Description, Category, CreatedAt) VALUES (@Id, @Title, @Description, @Category, DATEADD(minute, 330, GETUTCDATE()))`);
...
exports.createGoalTemplate = async (req, res) => {
    try {
        ...
        await pool.request()
            ...
            .input('Description', sql.NVarChar, req.body.description || '')
            .input('CreatedBy', sql.VarChar, req.user.Id || req.user._id)
            .query(`INSERT INTO GoalTemplates (Id, Name, Description, CreatedBy, CreatedAt) VALUES (@Id, @Name, @Description, @CreatedBy, DATEADD(minute, 330, GETUTCDATE()))`);

            ...
            await pool.request()
                ...
                .query(`
                    IF EXISTS (SELECT 1 FROM CalculatorAsins WHERE Id = @id)
                        UPDATE CalculatorAsins SET AsinCode = @asin, Title = @title, Category = @category, Price = @price, Weight = @weight, StapleLevel = @staple, Status = @status, UpdatedAt = dbo.GetEnvDate() WHERE Id = @id
                    ELSE
                        INSERT INTO CalculatorAsins (Id, AsinCode, Title, Category, Price, Weight, StapleLevel, Status, CreatedAt, UpdatedAt)
                        VALUES (@id, @asin, @title, @category, @price, @weight, @staple, @status, dbo.GetEnvDate(), dbo.GetEnvDate())
                `);
        }

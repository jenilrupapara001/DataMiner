    ...
    try {
      ...
      await pool.request()
        ...
        .query(`
          ...
          VALUES (@Id, @Title, @Description, @OwnerId, @StartDate, @EndDate, @Status, @Progress, dbo.GetEnvDate(), dbo.GetEnvDate())
        `);
  ...
  async calculateProgress(goalId) {
    try {
      ...
      await pool.request()
        ...
        .query(`
          UPDATE Goals 
          SET Progress = @progress, Status = @status, UpdatedAt = dbo.GetEnvDate()
          WHERE Id = @id
        `);

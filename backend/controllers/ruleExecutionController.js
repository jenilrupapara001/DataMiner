...
const acknowledgeAllAlerts = async (req, res) => {
  try {
    ...
    const result = await request
        ...
        .query(`
            UPDATE Alerts 
            SET Acknowledged = 1, AcknowledgedBy = @acknowledgedBy, AcknowledgedAt = dbo.GetEnvDate() 
            ${whereClause}
        `);
...
const toggleAlertRule = async (req, res) => {
  try {
    ...
    await pool.request()
        ...
        .query('UPDATE AlertRules SET IsActive = @isActive, UpdatedAt = dbo.GetEnvDate() WHERE Id = @id');
      ...
      if (shouldTrigger) {
        ...
        await pool.request()
            ...
            .query(`
                ...
                VALUES (@id, @sellerId, @type, @severity, @title, @message, 0, dbo.GetEnvDate())
            `);
        ...
        if (actionsConfig?.createTask) {
          ...
          await pool.request()
              ...
              .query(`
                  ...
                  VALUES (@id, @title, @description, @status, @priority, @type, @sellerId, @source, @sourceId, dbo.GetEnvDate())
              `);
...
const executeRule = async (req, res) => {
    try {
      ...
      await pool.request()
          ...
          .query('UPDATE AlertRules SET Execution = @execution, UpdatedAt = dbo.GetEnvDate() WHERE Id = @id');

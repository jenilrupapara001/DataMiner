export function getAchievementTier(pct: number, isAcos = false) {
    if (isAcos) {
        if (pct <= 100) return { label: 'Track (<=100%)', color: '#059669', bg: '#d1fae5' };
        return { label: 'Over (>100%)', color: '#ef4444', bg: '#fee2e2' };
    }

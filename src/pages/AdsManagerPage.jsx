  ...
  return (
    ...
    >
      <div className="d-flex flex-column" style={{ height: '85vh', overflow: 'hidden' }}>
        ...
        {/* Children Breakdown Table (Only shown for Parent rows) */}
        {rowData.isParent && rowData.children && rowData.children.length > 0 && (
          <div className="px-4 py-3 border-bottom bg-slate-50 shrink-0">
            <div className="smallest fw-bold text-indigo-800 mb-2" style={{ letterSpacing: '0.05em', textTransform: 'uppercase' }}>
              Child ASINs Performance breakdown ({rowData.children.length} items)
            </div>
            <Table
              dataSource={rowData.children.map((c, idx) => ({ ...c, key: idx }))}
              size="small"
              pagination={false}
              bordered
              scroll={{ y: 110 }}
              className="border rounded-2 overflow-hidden shadow-sm bg-white"
              columns={[
                {
                  title: 'ASIN',
                  dataIndex: 'asin',
                  ...
                  render: (asin, record) => (
                    <div className="d-flex flex-column">
                      <span className="font-monospace fw-bold text-zinc-900" style={{ fontSize: '10px' }}>{asin}</span>
                      <span className="text-zinc-500 font-monospace" style={{ fontSize: '9px' }}>{record.sku}</span>
                    </div>
                  )
                },
                {
                  ...
                  render: (title, record) => (
                    <div className="d-flex flex-column" style={{ maxWidth: '280px' }}>
                      <span className="fw-semibold text-zinc-800 text-truncate" style={{ fontSize: '10px' }} title={title}>{title}</span>
                      <span className="text-zinc-400" style={{ fontSize: '9px' }}>{record.brand} • {record.category}</span>
                    </div>
                  )
                },
                {
                  ...
                  render: (v) => <span className="fw-bold text-zinc-900 font-monospace" style={{ fontSize: '10px' }}>₹{Number(v || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                },
                {
                  title: 'AD SPEND',
                  dataIndex: 'spend',
                  ...
                  render: (v) => <span className="fw-bold text-rose-600 font-monospace" style={{ fontSize: '10px' }}>₹{Number(v || 0).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                },
                {
                  title: 'ACOS',
                  dataIndex: 'acos',
                  ...
                  render: (v) => <span className="fw-semibold font-monospace" style={{ fontSize: '10px', color: v > 30 ? '#ef4444' : '#10b981' }}>{Number(v || 0).toFixed(2)}%</span>
                },
                {
                  title: 'ROAS',
                  dataIndex: 'roas',
                  ...
                  render: (v) => <span className="fw-semibold text-indigo-600 font-monospace" style={{ fontSize: '10px' }}>{Number(v || 0).toFixed(2)}x</span>
                },
                {
                  title: 'ORDERS',
                  dataIndex: 'orders',
                  ...
                  render: (v) => <span className="fw-bold text-zinc-800 font-monospace" style={{ fontSize: '10px' }}>{v}</span>
                }
              ]}
            />
          </div>
